...
| ✎ \| `ManiaScoreHitEventGenerator`（**Obsolete**）               | ✎ \| 不产出 Statistics                             | ✎ \| → `RunHitEventsAsync`               | ✎ \| Session                   | ⛔ \| 默认 Purpose=**ForLive**                          | ✎ \| Graph 帧量化修复（已改直调 Service） | ✎ \| 无                     | ✎ \| OriginalHitEvents 覆盖         | ✎ \| 薄壳，勿新用                               |
